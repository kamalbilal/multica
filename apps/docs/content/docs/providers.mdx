...
<Callout type="info">
**Cursor (SDK)** is a fork-only provider (`cursor_sdk`) that runs the local `@cursor/sdk` executor instead of the `cursor-agent` CLI. It shares Cursor's MCP and `.cursor/skills/` paths with Cursor Agent. Setup is documented in `fork/docs/cursor-sdk-selfhost.md` at the repository root.
</Callout>
