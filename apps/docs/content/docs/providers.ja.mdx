...
<Callout type="info">
**Cursor (SDK)** はこの fork 専用のプロバイダー（`cursor_sdk`）で、`cursor-agent` CLI ではなくローカルの `@cursor/sdk` 実行器を使います。MCP と `.cursor/skills/` は Cursor Agent と共通です。設定はリポジトリ直下の `fork/docs/cursor-sdk-selfhost.md` を参照してください。
</Callout>
