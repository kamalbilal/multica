...
Expected output should show `running` status with detected agents (e.g. `claude`, `codex`, `copilot`, `opencode`, `openclaw`, `hermes`, `pi`, `cursor-agent`, `cursor_sdk`, `grok`). On this fork, `cursor_sdk` is probed separately (Node 22+ and the built executor — see [fork/docs/cursor-sdk-selfhost.md](fork/docs/cursor-sdk-selfhost.md)).

**If daemon fails to start:**
- Check logs: `multica daemon logs`
- If a port conflict occurs, the daemon may already be running under a different profile.
- If no agents are detected, ensure at least one AI CLI (`claude`, `codex`, `copilot`, `opencode`, `openclaw`, `hermes`, `pi`, `cursor-agent`, `cursor_sdk`, or `grok`) is installed. For `cursor_sdk`, build the executor per [fork/docs/cursor-sdk-selfhost.md](fork/docs/cursor-sdk-selfhost.md); other CLIs must be on `$PATH`.
...
Confirm:
1. Status is `running`
2. At least one agent is listed (e.g. `claude`, `codex`, `copilot`, `opencode`, `openclaw`, `hermes`, `pi`, `cursor-agent`, `cursor_sdk`, or `grok`)
3. At least one workspace is being watched

If the agents list is empty, tell the user:

> "The Multica daemon is running but no AI agent CLIs were detected. Please install at least one supported CLI (`claude`, `codex`, `copilot`, `opencode`, `openclaw`, `hermes`, `pi`, `cursor-agent`, `cursor_sdk`, or `grok`). For `cursor_sdk`, see [fork/docs/cursor-sdk-selfhost.md](fork/docs/cursor-sdk-selfhost.md). Then restart the daemon with `multica daemon stop && multica daemon start`."
